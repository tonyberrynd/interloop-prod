    ...
    Opportunity.create($scope.data.thisRecord).$promise
      .then(function(results){
                    ...
                    _.forEach($scope.data.selectedOwners, function(owner){
                        allPromises.push(Opportunity.owners.create(
                            {"id": thisRecord.id},
                            {
                    ...
                    _.forEach($scope.data.related, function(value){
                       var name = value.thisEntityType == 'Opportunity' ? value.firstName + ' ' + value.lastName : value.name;
                       //push in promise
                       allPromises.push(
                        RelationshipManager.linkEntity(thisRecord, value, "Opportunity", value.thisEntityType, 
                              {
                                "from": {
                    ...
                    return $q.allLimit(1, allPromises)
                      .then(function(data) {
                         Logger.info('Opportunity Created Succesfully');
