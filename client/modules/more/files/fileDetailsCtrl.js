...
angular.module('interloop.fileDetailsCtrl', [])
//declare dependencies
.controller('fileDetailsCtrl', function(
	$scope,
  $rootScope,
  $state,
  $stateParams,
...
function activate() {
  ...
  return Attachment.findOne({"filter": {"where": {"id": $stateParams.id}, "include": ['comments']}}).$promise
          .then(function(results){
            $scope.data.thisFile = results;
            //get comments
            $scope.data.comments = $scope.data.thisFile.comments;
          ...
          .catch(function(err){
            Logger.error('Error Retrieving File', 'Please try again in a moment')
          })
