...
angular.module('interloop.contactDetailsCtrl', [])
//declare dependencies
.controller('contactDetailsCtrl', function(
  $scope,
  $rootScope,
  $stateParams,
  $q,
...
//===========================================
  ...
  var contactId = $location.search().id || null;
  var shareLink = ShareLinkFactory.getShareLink('Contact', $stateParams.id);
...
function activate() {
  ...
  return Contact.findOne({"filter": {"where": {"id": $stateParams.id}, "include": ["owners", "sharedWith", "entities", "items", "activities", {"entities": ["comments"]}], "deleted": true}}).$promise
...
function updatePrimaryCompany(){
  $state.go('app.sidebar.contact.details');
...
function linkCompany(contact, company, updateGrid){
  return RelationshipManager.linkEntity(contact, company, "Contact", "Company",  
  {
  ...
  .then(function(results){
    // console.log(results);
      ...
      $scope.data.prevPrimaryCompany = results; //set new current since link was changed 
       //add this to entityLinks array of this contact
      ...
      if(updateGrid){   
          ...
          // $rootScope.$broadcast('OPP_UPDATED', {"id": contact.id}); //reload contact with changes 
          Logger.info('Contact Updated');
      }; 
  }); 
}; 

function unlinkCompany(contact, company, updateGrid) {
    //do this from company side to get back in format that matches relatedEntities 
    return RelationshipManager.unlinkEntity(contact, company, "Contact", "Company")
    .then(function(results){
      // console.log(results);
      ...
      //$rootScope.$broadcast('OPP_UPDATED', {"id": contact.id}); 
      if(updateGrid) { 
        //refresh grid
        gridManager.refreshView()
        // $rootScope.$broadcast('OPP_UPDATED', {"id": contact.id}); //reload contact with changes 
...
function viewRelationship(entityLink){
  ...
  $state.go('app.relationship-details', {'parentEntityType': 'contact', 'parentEntityId': $stateParams.id, 'entityLinkId': entityLink.entityId, 'entityLink': entityLink});
}
...
function goTo(entity, id){
  ...
  var currentState = {'entity': 'Contact', 'id': $stateParams.id }
 ...
 var newValue = {}
  ...
  if(field == 'primaryCompany') {
   ...
   //Save data for this contact 
    SidebarActions.saveData('Contact', $scope.data.thisRecord, newValue)
    .then(function(results){
...
*/
function cloneItem() {
  SidebarActions.cloneItem('Contact', $scope.data.thisRecord, $scope.data.owners);
}
...
*/
function deleteItem() {
  SidebarActions.deleteItem('Contact', $scope.data.thisRecord)
};
...
function unArchiveItem() {
  SidebarActions.unArchiveItem('Contact', $scope.data.thisRecord);
};
...
*/
function starItem() {
  SidebarActions.starItem('Contact', $scope.data.thisRecord)
    .then(function(results){
...
/*
Add / Remove Owners from the contact
*/
function addOwner() {
  var resolvedData = {
    entity: 'Contact',
...
*/
function logCall() {  
    SidebarActions.logCall('Contact', $scope.data.thisRecord)
};
...
*/
function addTask() {  
    SidebarActions.createTask('Contact', $scope.data.thisRecord)
};
...
*/
function addMeeting() {  
    SidebarActions.createMeeting('Contact', $scope.data.thisRecord)
};
...
function addActivity() {  
    SidebarActions.createActivity('Contact', $scope.data.thisRecord)
};
...
*/
function addNote() {  
    SidebarActions.createNote('Contact', $scope.data.thisRecord)
};
...
function triggerUpload() {
  angular.element('#ContactFileUpload').trigger('click');
}
...
function uploadFiles(files) {
  SidebarActions.uploadFiles('Contact', $scope.data.thisRecord, files)
}
...
*/ 
function manageTags() {
  SidebarActions.manageTags('Contact', $scope.data.thisRecord)
}
...
function removeTag(tag) {
  //remove tag from entity
  SidebarActions.removeTag('Contact', $scope.data.thisRecord, tag)
}
...
function manageRelationships() {
  SidebarActions.manageRelationships('Contact', $scope.data.thisRecord, $scope.data.pickLists)
}; 
...
function wonReason(value, oldValue) {
  SidebarActions.wonReason($scope.data.thisRecord, value, oldValue, _.find(contactConfig.picklists, { 'name': 'wonReasons' }))
}
...
function lostReason(value, oldValue) {
  SidebarActions.lostReason($scope.data.thisRecord, value, oldValue, _.find(contactConfig.picklists, { 'name': 'lostReasons' }))
}
