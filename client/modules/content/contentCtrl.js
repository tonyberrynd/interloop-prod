...
angular.module('interloop.contentCtrl', [])
...
//===========================================
  ...
  var contentId = $location.search().id || null;
...
function activate() {
  ...
  //------------------------------------------
  return View.find({"filter": {"where": {"entity": "Content"}}}).$promise
          ...
          //Get Current View
          //------------------------------------------
          return ViewManager.getThisView('Content', $scope.data.views, $stateParams.viewId, query, count)
              .then(function(results){
                //set this view
                ...
                if($scope.data.thisView.id == $rootScope.activeUser.defaultViews['Content']) {
                  $scope.data.default = true;
                }

                //Init Grid
                //------------------------------------------
                return gridManager.initGrid('Content', $scope.data.thisView)
                  .then(function(results){
                    //grid instance
                    ...
                    $timeout(function(){
                      ...
                      var customFields = _.filter($rootScope.customFields,function(o){
                        return _.includes(o.useWith, 'Content') && o.type !== 'divider';
                      })
                      //end fields
                      var endFields = EndFields || [];
                      //set up fields for filters
                      var fields = _.union(ContentFields, customFields, endFields);
                      //create filters
                      ...
                      //if link to individual content - open sidebar
                      //--------------------------------------
                      if(contentId) {
                        //open sidebar
                        SidebarRouter.openTo('Content', contentId)
                      }
...
function discardChanges() {
  $scope.data.filters = initialFilters(viewFilters, _.union(ContentFields, EndFields))
...
function bulkCreateView(){
    ...
    //creates static view that only includes these particular content
...
function changeView(view) {
  ...
  // change view
  $state.go('app.content', {"viewId": view.id}, {reload: 'app.content'});
}
...
function setUnsetDefault(value){
  if(value == true) {
      ViewManager.setDefault($scope.data.thisView, 'Content')
      .then(function(){
        $scope.data.default = true;
      })
  } else {
     ViewManager.clearDefault($scope.data.thisView, 'Content')
      .then(function(){
