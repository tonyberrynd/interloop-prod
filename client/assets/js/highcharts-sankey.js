(function (H) {
    ...
    H.seriesType('sankey', 'column', {
        ...
        pointAttribs: function (point, state) {
            ...
            var gradient = {
                  linearGradient : { x1: 1, y1: .5, x2: 0, y2: .5 },
                  stops : [
                    [0, point.fromColor || 'rgba(52, 152, 219,.5)'],
                    [1, point.toColor || 'rgba(41, 128, 185,.5)' ]
                  ]
                };

            return {
                fill: point.isNode ? point.cubeColor || '#ddd' : gradient
            };
        },
            ...
            each(this.nodeColumns, function (column) {
                each(column, function (node) {
                    ...
                    each(node.linksFrom, function (point) {
                        ...
                        if (!point.color) {
                            point.color = point.cubeColor || '#aaa'
                        }
                    });
                });
                left += colDistance;
