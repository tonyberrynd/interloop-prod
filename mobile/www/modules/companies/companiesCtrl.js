...
function activate() {
  ...
  return View.find({"filter": {"where": {"entity": "Company"}}}).$promise
          ...
          //Get Current View
          //------------------------------------------
          return ViewManager.getThisView('Company', $scope.data.views, $stateParams.viewId, query)
              .then(function(results){
                //set this view
                $scope.data.thisView = results;

                //get first page of data
                pageManager.getData(true, 'Company', $scope.data.thisView)
                .then(function(results){
...
function loadMoreData() {
	...
	if(!intializing){
    	...
    	pageManager.getData(false, 'Company', $scope.data.thisView)
    		.then(function(results){
 

    			$scope.data.companies = _.concat($scope.data.companies, results);
...
function changeView(view) {
  ...
  $timeout(function(){
      $state.go($state.current.name, {"viewId": view.id}, {reload: 'app.companies'});
    }, 250);
